"""DEPRECATED — the FlexTag 0.4 container ([[#tag ...]]: type ... [[/]]).

Kept so existing `.ft` files load during migration:

    from flextag import legacy
    view = legacy.load(path="old.ft")

This module is frozen: no fixes, no features, and it is REMOVED at 0.6. The
current format is markdown-hosted — see the package docstring and README.
"""
...
def _warn_legacy() -> None:
    import warnings
    warnings.warn(
        "flextag.legacy is the deprecated 0.4 container; it is removed at 0.6 — "
        "migrate .ft files to markdown (see README)",
...
def load(path=None, string=None, dir=None, filter_query=None,
         validate=True, strict=False, settings=None, recursive=True):
    """The 0.4 top-level loader, unchanged: `legacy.load(path=...)` reads `.ft`."""
    return FlexTag.load(path=path, string=string, dir=dir, filter_query=filter_query,
                        validate=validate, strict=strict, settings=settings,
                        recursive=recursive)
